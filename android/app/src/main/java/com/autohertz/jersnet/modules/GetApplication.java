...
import com.autohertz.jersnet.utils.Image;

public class GetApplication extends ReactContextBaseJavaModule {

  private final ReactApplicationContext reactContext;

  public GetApplication(ReactApplicationContext reactContext) {
    super(reactContext);
  ...
  public void listApps(Promise promise) {
    try {
      ...
      for (int i = 0; i < pList.size(); i++) {
        ...
        boolean isUserApps = (packageInfo.applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) == 0;
        ...
        appInfo.putBoolean("isUserApp", isUserApps);

        Drawable icon = pm.getApplicationIcon(packageInfo.applicationInfo);
        appInfo.putString("icon", Image.convert(icon));
